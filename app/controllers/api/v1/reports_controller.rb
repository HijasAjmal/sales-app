class Api::V1::ReportsController < ApplicationController
	...
	def generate_user_report
		if params[:report][:value] == 0
			reports = PurchaseStore.generate_user_report(params[:report][:start_date], params[:report][:end_date], params[:report][:record_id])
		elsif params[:report][:value] == 1
			reports = Store.generate_user_report(params[:report][:start_date], params[:report][:end_date], params[:report][:record_id])
		end
		if reports.present?
			render json: { reports: reports, request_status: 200, request_message: "" }
		else
